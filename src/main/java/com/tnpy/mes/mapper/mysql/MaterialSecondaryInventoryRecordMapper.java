...
public interface MaterialSecondaryInventoryRecordMapper {
    ...
    @Insert("insert into tb_materialsecondaryinventoryrecord \n" +
            "select UUID(),g.id,#{plantID},#{processID},(ifnull(currentNum,0) +ifnull(grantNum,0)  -ifnull(expendNum,0)   +ifnull(onlineNum,0) + ifnull(mergeNum,0)  - ifnull(repairNum,0) ) as newNum,ifnull(currentNum,0),now(),ifnull(grantNum,0) ,0," +
            ...
            "( select materialID,sum(CASE status WHEN '1' THEN sumNum ELSE 0 END ) 'onlineNum', sum(CASE status WHEN '2' THEN sumNum ELSE 0 END ) 'mergeNum', sum(CASE status WHEN '3' THEN sumNum ELSE 0 END ) 'repairNum' from (\n" +
            "select materialID,sum(materialNum) as sumNum,status from tb_onlinematerialrecord where  plantID = #{plantID} and processID = #{processID} and updateTime >= #{startTime} and updateTime < #{endTime} group by status,materialID ) a GROUP BY materialID\n" +
