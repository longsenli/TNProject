...
public interface DailyProductionAndWageDetailMapper {
    ...
    @Select(" select c.*,uuid() as id,ifnull(d.wage,0) as univalence,round(c.shelfProduction * ifnull(d.wage,0),2) as wage from (\n" +
            "select a.plantID,a.processID,a.lineID,a.worklocationID,a.materialName,a.materialID,a.totalProduction,b.staffID,b.staffName,b.classType1,b.classType2,DATE_FORMAT( b.dayTime,'%Y-%m-%d') as dayTime,a.totalProduction as shelfProduction from (\n" +
            ...
            " on a.lineID = b.lineID and b.verifierID is not null and b.classType1 = #{classType} and b.dayTime = #{dayTime} ) c left join sys_material d on c.materialID= d.id order by lineID\n" )
    ...
    @Select(" select c.*,uuid() as id,ifnull(d.wage,0) as univalence,round(c.shelfProduction * ifnull(d.wage,0),2) as wage from (\n" +
            ...
            " on a.worklocationID = b.worklocationID and b.verifierID is not null and b.classType1 = #{classType} and b.dayTime = #{dayTime} ) c left join sys_material d on c.materialID= d.id order by lineID\n" )
    ...
    @Select("  select c.*,uuid() as id,ifnull(d.wage,0) as univalence,round(c.shelfProduction * ifnull(d.wage,0),2) as wage from (\n" +
            ...
            " where plantID = #{plantID} usedOrderID like  ${orderInfo} group by workLocation,materialName\n" +
            " ) a left join tb_staffattendancedetail b \n" +
            " on a.lineID = b.lineID and b.verifierID is not null and b.classType1 = #{classType} and b.dayTime = #{dayTime} ) c left join sys_material d on c.materialID= d.id order by worklocationID\n" )
    ...
    @Select("select plantID,processID,lineID,worklocationID,totalProduction,staffName,shelfProduction,univalence,wage,classType1,classType2,DATE_FORMAT( dayTime,'%Y-%m-%d') as dayTime,verifierName," +
