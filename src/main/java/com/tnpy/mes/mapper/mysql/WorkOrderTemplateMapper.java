...
public interface WorkOrderTemplateMapper {
    ...
    @Select(" SELECT * FROM( \r\n" + 
    		"    (SELECT  IFNULL(m.materialNameInfo, '无') AS '物料型号', CONVERT (IFNULL(m.number, '无')  USING utf8) AS '数量', IFNULL(m.subOrderID, '无') AS '入库工单', IFNULL(m.inputer, '无') AS '入库人员姓名', IFNULL(pt1.`name`, '无') AS '入库厂区', IFNULL(pc1.`name`, '无') AS '入库工序', IFNULL(pl1.`name`, '无') AS '入库产线',"
    		+ " IFNULL(w1.`name`, '无') AS '入库工位', IFNULL(m.inputTime, '无') AS '入库时间',  IFNULL(m.expendOrderID, '无') AS '投料工单', IFNULL(m.outputer, '无') AS '投料人',  IFNULL(pc2.`name`, '无') AS '投料工序', IFNULL(pl2.`name`, '无') AS '投料产线', IFNULL(w2.`name`, '无') AS '投料工位', IFNULL(m.outputTime, '无') AS '投料时间'\r\n" + 
    		...
    		"        ON m.outputWorkLocationID = w2.id\r\n" + 
    		"    WHERE ${filter})\r\n" + 
    		"    UNION\r\n" + 
    		"    ALL \r\n" + 
    		"        (SELECT  IFNULL(m.materialNameInfo, '无') AS '物料型号', CONVERT ( sum(m.number) USING utf8)  AS '数量', '总计' AS '入库工单', '' AS '入库人员姓名', '' AS '入库厂区', '' AS '入库工序', '' AS '入库产线', '' AS '入库工位', '' AS '入库时间', '' AS '投料工单', '' AS '投料人', '' AS '投料工序', '' AS '投料产线', '' AS '投料工位', '' AS '投料时间'\r\n" + 
    		"        FROM tb_materialrecord m\r\n" + 
    		"        WHERE ${filter}\r\n" + 
    		"        GROUP BY  m.materialID ) ) rsall\r\n" + 
    		"    ORDER BY  rsall.`投料产线` ,rsall.`投料时间`, rsall.`投料工单`, rsall.`入库工单` , rsall.`物料型号` limit 1000")
